...
addEventListener('submit',(event)=>{
    ...
    const cant_MiFormularios=document.getElementById('cant_MiFormularios').value
    const plan_select=parseInt(document.querySelector('#plan-select option:checked').value)
    const email=document.getElementById('cant_MiFormularios').value
    const photo=document.getElementById('photo').value
    const message=document.getElementById('form-mensaje')

    if( /^\s*$/.test(fullname) ||
        /^\s*$/.test(cant_MiFormularios) ||
    ...
    if( cant_MiFormularios <= 0 || cant_MiFormularios > 10 ) {
        message.textContent="Cantidad de MiFormularios entre 1 y 10."
        return
    }
